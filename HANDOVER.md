...
Continued through the rest of the low/medium-risk list in the same session:
- **`communities`+`events` domains** → `lib/muse-actions/communities.ts`: join-community, leave-community, create-community, create-event, rsvp, cancel-rsvp (6 handlers).
- **`sessions/bookings` domain** (the biggest remaining chunk) → `lib/muse-actions/sessions.ts`: book-session, create-session, respond-booking, cancel-booking, complete-booking, submit-review, respond-checkin, get-checkins, share-safety-details, save-safety-profile, get-safety-profile, get-prompts, save-prompt-response, get-prompt-responses (14 handlers, incl. the Stripe payment capture/cancel logic for booking completion/cancellation).

route.ts: 2032 → 1207 lines across five commits (825 lines out, ~41% smaller). Each commit verified independently: `tsc --noEmit` clean, 157/157 tests, `npm run build` clean. Dispatch registry, POST URL, and every frontend call site unchanged throughout — zero blank-screen risk per the proven pattern. Dropped now-dead imports as each domain left the file (askMuseAI, scanWithRekognition, logScan, Stripe, parseRateToCents).

**Deliberately stopping here and handing back.** What's left — profile, match, message, feed, forum, connect, then the GET `type=...` switch — is exactly the set wyzmind's own handoff flagged as highest-traffic/highest-regression-risk and said to save for last. Rather than rush through six domains covering the bulk of the 102 frontend call sites in one sitting, better to let this batch get reviewed/merged/deployed first, then continue the last stretch fresh. The pattern is proven across 6 domains now (admin, disclosures/strikes, communities/events, sessions/bookings, plus wyzmind's own quests/albums/feedback) — the remaining cut is mechanical, just higher-stakes to rush.

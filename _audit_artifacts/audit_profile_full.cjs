...
(async () => {
  ...
  // Scroll to portfolio section - try multiple positions
  for (const pos of [400, 500, 600, 700]) {
    await page.evaluate((p) => {
      const scroll = document.querySelector('.profile-scroll');
      if (scroll) scroll.scrollTop = p;
    }, pos);
    await page.waitForTimeout(500);
  }

  // Take full page screenshot
  await page.screenshot({ path: path.join(__dirname, 'final_profile_full.png'), fullPage: true });
  console.log('Profile full page captured');

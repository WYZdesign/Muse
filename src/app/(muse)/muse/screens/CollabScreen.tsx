            ...
            <div key={brief.id} className="brief-card" style={{ position: "relative" }}>
              ...
              <div className="brief-header" style={{ flexWrap: "wrap", gap: 6 }}>
                ...
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 4, width: "100%", justifyContent: "center" }}>
                  {brief.cat === "tfp" && <span role="button" tabIndex={0} className="brief-tag" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "TFP", desc: "Trade for print — this brief offers portfolio/collaboration credit instead of cash payment.", icon: "🔄", color: "var(--mint)" })} style={{ background: "rgba(152,251,152,0.15)", borderColor: "rgba(152,251,152,0.3)", color: "var(--mint)", cursor: "pointer" }}>TFP</span>}
                  {brief.cat === "paid" && <span role="button" tabIndex={0} className="brief-tag" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "Paid", desc: "This brief pays — there's a cash budget attached.", icon: "💵", color: "var(--gold)" })} style={{ background: "rgba(255,215,0,0.12)", borderColor: "rgba(255,215,0,0.2)", color: "var(--gold)", cursor: "pointer" }}>Paid</span>}
                  {brief.cat === "opencall" && <span role="button" tabIndex={0} className="brief-tag" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "Open Call", desc: "An open call — anyone can submit or audition for this opportunity.", icon: "📣", color: "#1E88E5" })} style={{ background: "rgba(30,136,229,0.2)", borderColor: "rgba(30,136,229,0.4)", color: "#1E88E5", cursor: "pointer" }}>Open Call</span>}
                  {brief.cat === "concept" && <span role="button" tabIndex={0} className="brief-tag" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "Ideas", desc: "A concept or idea looking for collaborators to bring it to life — no payment, just creative interest.", icon: "💡", color: "var(--lavender)" })} style={{ background: "rgba(212,165,255,0.12)", borderColor: "rgba(212,165,255,0.25)", color: "var(--lavender)", cursor: "pointer" }}>Ideas</span>}
                  {brief.urgent && <span role="button" tabIndex={0} className="brief-tag" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "Urgent", desc: "Time-sensitive — the poster needs someone fast.", icon: "⚡", color: "var(--coral)" })} style={{ background: "rgba(255,107,107,0.15)", borderColor: "rgba(255,107,107,0.3)", color: "var(--coral)", cursor: "pointer" }}>Urgent</span>}
                  {brief.nsfw && <span role="button" tabIndex={0} className="brief-tag" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "18+", desc: "Adult / NSFW content — only shown to verified adults.", icon: "🔞", color: "var(--sunset)" })} style={{ background: "rgba(255,107,107,0.15)", borderColor: "rgba(255,107,107,0.3)", color: "var(--sunset)", cursor: "pointer" }}>18+</span>}
                </div>
              </div>

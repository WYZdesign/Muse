          ...
          <div className={"discover-hdr-actions" + (discoverSearchOpen ? " search-open" : "")} style={{ display: "flex", gap: 4, alignItems: "center" }}>
            ...
            {!discoverSearchOpen && (
              <>
                ...
                <button className={"hdr-btn" + (boostActive ? " hdr-btn-glow" : "")} onClick={() => { if (boostActive) { setBoostActive(false); setBoostEnd(0); try { safeRemoveItem?.("muse_boost"); } catch {} showToast("Boost off"); return; } const end = Date.now() + 1800000; if (demo) { setBoostActive(true); setBoostEnd(end); try { safeSetItem?.("muse_boost", "" + end); } catch {} showToast("Demo boost on for 30 min — no real promotion was purchased."); return; } apiFetch("/api/muse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "boost" }) }).then((r: any) => { if (r.ok) { setBoostActive(true); setBoostEnd(end); try { safeSetItem?.("muse_boost", "" + end); } catch {} showToast("Boost on for 30 min!"); } else { r.json?.().then((d: any) => showToast((d && (d.error || "Boost unavailable")) || "Boost unavailable")).catch(() => showToast("Boost unavailable")); } }).catch(() => showToast("Boost unavailable")); }} style={{ width: 44, height: 44 }} aria-label="Boost"><FiZap size={16} /></button>
              </>
            )}
          </div>
        </div>

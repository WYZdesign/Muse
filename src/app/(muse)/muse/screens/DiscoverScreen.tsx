                                    ...
                                    <div>
                                      ...
                                      >
                                        ...
                                        {albumPhotos.length > 1 && (
                                          <>
                                             <div role="button" tabIndex={0} aria-label="Previous portfolio photo" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.stopPropagation(); setPortfolioPhotoIdx(p => Math.max(0, p - 1)); } }} onClick={(e) => { e.stopPropagation(); setPortfolioPhotoIdx(p => Math.max(0, p - 1)); }} style={{ position: "absolute", left: 0, top: 0, width: "30%", height: "100%", zIndex: 2 }} />
                                             <div role="button" tabIndex={0} aria-label="Next portfolio photo" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.stopPropagation(); setPortfolioPhotoIdx(p => Math.min(albumPhotos.length - 1, p + 1)); } }} onClick={(e) => { e.stopPropagation(); setPortfolioPhotoIdx(p => Math.min(albumPhotos.length - 1, p + 1)); }} style={{ position: "absolute", right: 0, top: 0, width: "30%", height: "100%", zIndex: 2 }} />
                                          </>
                                        )}
                                        {/* Left/Right arrows */}
                                        {albumPhotos.length > 1 && (

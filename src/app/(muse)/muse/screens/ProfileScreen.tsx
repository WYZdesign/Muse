  ...
  return (
    <div className={"screen-el" + (screen === "profile" ? " active" : "")} data-screen="profile">
      ...
      <div className="profile-scroll">
        {/* 2026-09-15: Torreé asked to drop the "Unlimited Likes" bubble/popup
            entirely — unlimited likes should just work silently (isUnlimited
            is already unconditionally true in page.tsx), not be announced
            with a dismissible pill. */}
        <div className="profile-top">
          <div className="profile-avatar-wrap">
            ...
            {/* Hoolah-hoop: halo (.profile-ring) is the CSS-default 115px (Session 85:
                ...
                reverted back to 115px/125px, then the gap tightened ~10% to 124px — a
                ~4.5px gap past the halo's edge on every side, close enough to read as
        ...
        <div className="section">
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            ...
            {checkProfileBadges(currentUser.stats, currentUser.createdAt).map(b => <span key={b.name} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: b.name, desc: b.desc || "A badge earned for activity on Muse.", icon: b.icon, color: b.color })} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 600, padding: "6px 12px", borderRadius: 99, background: `${b.color}20`, border: `1px solid ${b.color}40`, color: b.color, cursor: "pointer" }}>{b.icon} {b.name}</span>)}
          </div>
        </div>
        <div className="section">

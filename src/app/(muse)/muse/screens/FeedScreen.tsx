            ...
            return (
              <div key={post.id} className="conn-card" style={{ flexDirection: "column", margin: "0 20px 14px", padding: 0, overflow: "hidden", position: "relative" }}>
                ...
                <div style={{ display: "flex", gap: 6, padding: "10px 12px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <button className="feed-action-btn" style={{ flex: 1, minWidth: 0, height: 42, background: "transparent", border: "1px solid rgba(255,255,255,0.08)", color: "var(--text2)", cursor: "pointer", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "0 4px", borderRadius: 14, transition: "all .2s ease", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} onClick={() => { const url = typeof window !== "undefined" ? window.location.href : ""; if (typeof navigator !== "undefined" && navigator.share) { navigator.share({ title: post.author || "Muse post", text: (post.text || "").slice(0, 200), url }).catch(() => {}); } else { try { navigator.clipboard?.writeText(url); showToast("Link copied!"); } catch { showToast("Link copied!"); } } }}>Share</button>
                  <button className="feed-action-btn" title="Save for later" style={{ flex: 1, minWidth: 0, height: 42, background: post.saved ? "rgba(255,215,0,0.16)" : "rgba(255,255,255,0.04)", border: post.saved ? "1.5px solid rgba(255,215,0,0.35)" : "1px solid rgba(255,255,255,0.08)", color: post.saved ? "#FFD700" : "var(--text2)", cursor: "pointer", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "0 4px", borderRadius: 14, transition: "all .2s ease", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} onClick={() => { const newSaved = !post.saved; if (feedPostsStatic.some(p => p.id === post.id)) { setFeedPostsStatic(prev => prev.map(p => p.id === post.id ? ({ ...p, saved: newSaved }) : p)); } else { updateFeedPostState(post.id, p => ({ ...p, saved: newSaved })); } showToast(newSaved ? "Saved ✓" : "Removed from saves"); }}>{post.saved ? "Saved" : "Save"}</button>
                </div>
                {replyingTo === post.id && (

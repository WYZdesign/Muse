          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flexShrink: 0, width: 52 }}>
            {editingStatus ? (
              <input
                ...
                maxLength={80}
                placeholder="Status"
                style={{ width: 52, textAlign: "center", margin: 0, fontSize: 10, padding: "4px 4px", background: "var(--glass)", border: "1px solid rgba(255,215,0,0.3)", color: "#FFD700", borderRadius: 99 }}
              />
            ) : (
              // Pill shows just the status's first glyph (title carries the full text) —
              // at a 52px pill width, rendering more than one character reliably clips
              // or overflows depending on the character set, so a single glyph is the
              // one size that always fits cleanly.
              <div
                onClick={() => setEditingStatus(true)}
                title={userStatus || "Set status"}
                style={{ width: 52, height: 20, lineHeight: "20px", textAlign: "center", cursor: "pointer", fontSize: 12, color: "var(--gold)", fontWeight: 700, padding: "0 2px", background: "rgba(255,215,0,0.08)", border: "1px solid rgba(255,215,0,0.15)", borderRadius: 99, overflow: "hidden", whiteSpace: "nowrap" }}
              >

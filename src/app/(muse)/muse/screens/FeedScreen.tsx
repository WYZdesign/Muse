...
export const FeedScreen = memo(function FeedScreen({
  ...
  const filteredFeedPosts = allFeedPosts.filter((post) => {
    ...
    if (feedFilter === "photos") return post.type === "photo" || Boolean(post.img);
    if (feedFilter === "bts") return post.isBts || (post.type === "video" && !post.text);
...
<div style={{ margin: "0 20px 12px", padding: "12px 0", display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                onClick={async () => {
                  ...
                  if (feedText.trim() || feedMedia.length || feedClip) {
                    ...
                    try {
                      ...
                      setFeedPosts(prev => prev.filter(p => p.id !== optimisticId));
                      ...
                      <div style={{ position: "relative" }}>
                        <input
                          ...
                          <button
                            ...
                            style={{ position: "absolute", right: 0, top: "50%", transform: "translateY(-50%)", width: 44, height: 44, borderRadius: "50%", border: "none", background: (postCommentTexts[post.id] || "").trim() ? "linear-gradient(135deg,var(--coral),var(--pink))" : "rgba(255,255,255,0.06)", color: (postCommentTexts[post.id] || "").trim() ? "#fff" : "rgba(255,255,255,0.25)", cursor: (postCommentTexts[post.id] || "").trim() ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s" }}
      ...
      {(() => {
        ...
        const sendDetailReply = async () => {
          ...
          setPostCommentTexts(prev => ({ ...prev, [dp.id]: "" }));
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ position: "relative" }}>
                  ...
                  <button
                    ...
                    style={{ position: "absolute", right: 0, top: "50%", transform: "translateY(-50%)", width: 44, height: 44, borderRadius: "50%", border: "none", background: (postCommentTexts[dp.id] || "").trim() ? "linear-gradient(135deg,var(--coral),var(--pink))" : "rgba(255,255,255,0.06)", color: (postCommentTexts[dp.id] || "").trim() ? "#fff" : "rgba(255,255,255,0.25)", cursor: (postCommentTexts[dp.id] || "").trim() ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s" }}
                  ><FiSend size={14} /></button>
                </div>
              </div>
            </div>

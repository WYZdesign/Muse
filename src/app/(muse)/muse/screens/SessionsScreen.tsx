...
<div style={{ flex: 1, overflowY: "auto", padding: "0 16px 80px" }}>
            ...
            {(() => {
              ...
              <div key={s.id} className="conn-card" style={{ marginBottom: 10, padding: 0, overflow: "hidden", flexDirection: "row", alignItems: "stretch", position: "relative" }}>
                {/* Audit fix: this was the only "⋯" report button in the app hardcoded
                    to full-opacity white (border/text #fff on transparent), making it
                ...
                <button aria-label="Report session" title="Report" onClick={() => { setReportTarget({ id: s.id, type: "session", name: s.name || "session" }); setShowReport(true); }} style={{ position: "absolute", top: 4, right: 4, zIndex: 2, width: 44, height: 44, borderRadius: "50%", border: "1px solid var(--border-subtle)", background: "rgba(20,15,25,0.15)", color: "var(--muted)", fontSize: 11, lineHeight: 1, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>⋯</button>
                    ...
                    {(() => {
                      ...
                      return (
                        <span
                          ...
                          onClick={(e) => { e.stopPropagation(); setBadgeInfo({ name: tier.label, desc: tier.key === "elite" ? "Muse's top tier — 25+ completed sessions and a 4.8+ average rating, among the most trusted hosts on Muse." : tier.key === "top" ? "Top Rated — 10+ completed sessions and a 4.5+ average rating, a proven and reliable host." : "Rising Muses — 3+ completed sessions and a 4.0+ average rating, building a strong track record.", icon: tier.icon, color: tier.color }); }}
                          onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.stopPropagation(); setBadgeInfo({ name: tier.label, desc: tier.key === "elite" ? "Muse's top tier — 25+ completed sessions and a 4.8+ average rating, among the most trusted hosts on Muse." : tier.key === "top" ? "Top Rated — 10+ completed sessions and a 4.5+ average rating, a proven and reliable host." : "Rising Muses — 3+ completed sessions and a 4.0+ average rating, building a strong track record.", icon: tier.icon, color: tier.color }); } }}
          ...
          {(s.skills || []).map((sk: string) => <span key={sk} className="conn-tag" role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.stopPropagation(); e.currentTarget.click(); } }} onClick={(e) => { e.stopPropagation(); setBadgeInfo({ name: sk, desc: `A skill covered in this session — what you'll work on or learn.`, icon: "🛠", color: "#90caf9" }); }} style={{ fontSize: 10, padding: "3px 8px", cursor: "pointer" }}>{sk}</span>)}
                  </div>
                  ...
                  <div className="conn-content" style={{ flex: 1, padding: 14, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      <div className="conn-name" style={{ fontSize: 15, display: "flex", alignItems: "center", gap: 6 }}>
                        {host.name || "Host"}
                        {host.verified && <span className="card-verified-mark" style={{ fontSize: 13, cursor: "pointer" }} title="Identity verified" role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "Verified Host", desc: "Identity verified by Muses by WYZ — we confirmed this host's government ID and credentials.", icon: "✓", color: "#FFD700" })}>✓</span>}
                      </div>
                      <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                        {pp && <span role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: pp.label, desc: pp.label === "Payment held" ? "Your payment is held in escrow securely until the session is completed." : pp.label === "Paid" ? "Payment completed successfully through secure checkout." : pp.label === "Refunded" ? "This payment was refunded to the client." : pp.label === "Payment failed" ? "The payment attempt failed — no money was taken." : "Payment confirmation state.", icon: "💳", color: pp.colors.c })} style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 99, background: pp.colors.bg, color: pp.colors.c, border: `1px solid ${pp.colors.bd}`, whiteSpace: "nowrap", cursor: "pointer" }}>{pp.label}</span>}
                        <span role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: label, desc: label === "Awaiting host" ? "Waiting for the host to confirm your request." : label === "Confirmed" ? "The host has confirmed this booking." : label === "Completed" ? "This session has been completed." : label === "Cancelled" ? "This booking was cancelled." : "Current status of this booking.", icon: "📋", color: sc.c })} style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 99, background: sc.bg, color: sc.c, border: `1px solid ${sc.bd}`, whiteSpace: "nowrap", cursor: "pointer" }}>{label}</span>
                      </div>
                    </div>
                ...
                <div key={b.id} className="conn-card" style={{ marginBottom: 10, padding: 0, overflow: "hidden", flexDirection: "row", alignItems: "stretch" }}>
                  ...
                  <div className="conn-content" style={{ flex: 1, padding: 14, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      <div className="conn-name" style={{ fontSize: 15, display: "flex", alignItems: "center", gap: 6 }}>
                        {booker.name || "Booker"}
                    {booker.verified && <span className="card-verified-mark" style={{ fontSize: 13, cursor: "pointer" }} title="Identity verified" role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: "Verified Member", desc: "Identity verified by Muses by WYZ — we confirmed this member's government ID and credentials.", icon: "✓", color: "#FFD700" })}>✓</span>}
                      </div>
                      <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                    {pp && <span role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: pp.label, desc: pp.label === "Payment held" ? "Payment is held in escrow securely until the session is completed." : pp.label === "Paid" ? "Payment completed successfully through secure checkout." : pp.label === "Refunded" ? "This payment was refunded to the client." : pp.label === "Payment failed" ? "The payment attempt failed — no money was taken." : "Payment confirmation state.", icon: "💳", color: pp.colors.c })} style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 99, background: pp.colors.bg, color: pp.colors.c, border: `1px solid ${pp.colors.bd}`, whiteSpace: "nowrap", cursor: "pointer" }}>{pp.label}</span>}
                    <span role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setBadgeInfo({ name: label, desc: label === "Pending" ? "Waiting for you to accept or decline this request." : label === "Confirmed" ? "You've confirmed this booking." : label === "Completed" ? "This session has been completed." : label === "Cancelled" ? "This booking was cancelled." : "Current status of this request.", icon: "📋", color: sc.c })} style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 99, background: sc.bg, color: sc.c, border: `1px solid ${sc.bd}`, whiteSpace: "nowrap", cursor: "pointer" }}>{label}</span>
                      </div>
                    </div>
                    <div className="conn-meta" style={{ fontSize: 12 }}>{sess.title || "Session"} · {sess.rate || "Rate TBD"}</div>
                    <div style={{ display: "flex", gap: 8, marginTop: 8 }}>

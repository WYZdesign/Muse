                          ...
                          {groupMembers.slice(0, 20).map(m => {
                            ...
                            return (
                              <div key={m.user_id} style={{ display: "flex", alignItems: "center", gap: 10, position: "relative" }}>
                                ...
                                {roleBadge && <button type="button" onClick={(e) => { e.stopPropagation(); setBadgeInfo({ name: m.role, desc: m.role === "admin" ? "This member administers the community — they manage the group and its rules." : "This member is a moderator — they help keep the community welcoming and on-topic.", icon: m.role === "admin" ? "🛡" : "✋", color: roleBadge.c }); }} style={{ minHeight: 44, fontSize: 10, padding: "2px 9px", borderRadius: 99, background: roleBadge.bg, border: `1px solid ${roleBadge.bd}`, color: roleBadge.c, fontWeight: 700, textTransform: "capitalize", cursor: "pointer" }}>{m.role}</button>}
            ...
            {commTab === "groups" ? (
              <>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--text2)", marginBottom: 12, cursor: "pointer" }}>
                  <input type="checkbox" aria-label="18+ / NSFW group" checked={form.isNsfw} onChange={e => setForm(p => ({ ...p, isNsfw: e.target.checked }))} style={{ accentColor: "#ffd700", width: 16, height: 16 }} /> 18+ / NSFW group
                </label>
              </>
            ) : (
              <>
            ...
            <div className="conn-content" style={{ width: "100%", padding: 14, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", marginTop: 8, marginBottom: 8 }}>
                {(() => {
                  ...
                  return badges.map(b => <button type="button" key={b.t} onClick={(e) => { e.stopPropagation(); setBadgeInfo({ name: b.t, desc: b.t === "✓ Joined" ? "You've joined this community." : b.t === "Open to Join" ? "This community is open — anyone can join." : b.t === "18+" ? "Adult / NSFW community — only shown to verified adults." : b.t === "Large" ? "A large community with 500+ members." : b.t === "Growing" ? "A growing community with 100+ members." : b.t === "Small" ? "A small community with under 100 members." : "The category this community belongs to.", icon: b.t === "✓ Joined" ? "✅" : b.t === "Open to Join" ? "👋" : b.t === "18+" ? "🔞" : b.t === "Large" ? "👥" : b.t === "Growing" ? "🌱" : b.t === "Small" ? "🏷" : "🏷", color: b.c });}} style={{ minHeight: 44, fontSize: 11, padding: "3px 10px", borderRadius: 99, background: b.bg, border: `1px solid ${b.bd}`, color: b.c, fontWeight: 600, cursor: "pointer" }}>{b.t}</button>);
                })()}
              </div>
            </div>
            ...
            <div style={{ width: "100%", padding: 16, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", marginTop: 10, width: "100%" }}>
                {(() => {
                  ...
                  return badges.map(b => <button type="button" key={b.t} onClick={(e) => { e.stopPropagation(); setBadgeInfo({ name: b.t, desc: b.t === "Today" ? "This event is happening today." : b.t === "This week" ? "This event is happening this week." : b.t === "Upcoming" ? "This event is scheduled for a later date." : b.t === "Online" ? "This event takes place online / virtually." : b.t === "In person" ? "This event is held in person at a live location." : b.t === "✓ Going" ? "You've RSVP'd to this event." : b.t === "RSVP open" ? "This event is open for RSVPs right now." : b.t === "18+" ? "Adult / NSFW event — only shown to verified adults." : "This event's status.", icon: b.t === "Today" ? "📌" : b.t === "This week" ? "📅" : b.t === "Upcoming" ? "🗓" : b.t === "Online" ? "💻" : b.t === "In person" ? "📍" : b.t === "✓ Going" ? "✅" : b.t === "RSVP open" ? "🎟" : "🔞", color: b.c });}} style={{ minHeight: 44, fontSize: 11, padding: "3px 10px", borderRadius: 99, background: b.bg, border: `1px solid ${b.bd}`, color: b.c, fontWeight: 600, cursor: "pointer" }}>{b.t}</button>);
                })()}
              </div>
            </div>

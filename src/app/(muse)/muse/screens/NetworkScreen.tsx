              ...
              <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)", borderRadius: 16, overflow: "hidden", background: "linear-gradient(135deg,#1a0a2e 0%,#2d1b4e 50%,#1a0a2e 100%)", border: "1px solid rgba(255,215,0,0.2)" }}>
                ...
                <div style={{ position: "relative", padding: "22px", display: "flex", flexDirection: "column", gap: 12, height: "100%" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: "auto", paddingTop: 12 }}>
                    <button className={connectedIds.has(p.id) ? "btn btn-outline" : "btn btn-gold"} style={{ flex: 1, padding: "12px 0", fontSize: 13, fontWeight: 700, borderRadius: 12 }} onClick={async (e) => { e.stopPropagation(); try { const r = await apiFetch("/api/muse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "connect", targetId: p.id }) }); if (!r.ok) throw new Error("failed"); setConnectedIds(prev => { const n = new Set(prev); n.add(p.id); return n; }); showToast("Connection request sent to " + p.name + "!"); } catch { showToast("Couldn't connect — try again"); } }}>{connectedIds.has(p.id) ? "✓ Requested" : "Connect"}</button>
              ...
              <div className="modal-overlay" style={{ position: "fixed", zIndex: 400 }}>
                ...
                <div className="modal-body" style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button className="btn btn-gold" style={{ flex: 1, padding: "14px 0", fontSize: 14, fontWeight: 700, borderRadius: 12 }} onClick={async () => { if (!newPostTitle.trim()) return; const title = newPostTitle.trim(); const body = newPostBody.trim(); try { const r = await apiFetch("/api/muse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "forum", title, body, userId: currentUser.id }) }); if (!r.ok) { const d = await r.json().catch(() => ({} as any)); showToast(d.code === "SAFETY_BLOCK" ? "Post blocked by safety policy" : "Failed to post"); return; } setForumPosts(prev => [{ id: uid(), title, body, author: currentUser.name, avatar: currentUser.avatar, votes: 1, comments: [], cat: "General", time: "Just now", pinned: false }, ...prev]); setNewPostTitle(""); setNewPostBody(""); setShowNewPost(false); showToast("Posted!"); } catch { showToast("Failed to post"); } }}>Post</button>
              ...
              <div key={post.id} className="conn-card" style={{ flexDirection: "column", marginBottom: 8, padding: 14 }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, minWidth: 36 }}>
                    <button style={{ background: "none", border: "none", color: post.votes > 0 ? "var(--gold)" : "var(--muted)", cursor: "pointer", fontSize: 18, padding: 0 }} onClick={() => { setForumPosts(prev => prev.map(p => p.id === post.id ? { ...p, votes: p.votes + 1 } : p)); apiFetch("/api/muse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "forum", type: "vote", postId: post.id, direction: "up" }) }).catch(() => {}); }}>▲</button>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>{post.votes}</span>
                    <button style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 18, padding: 0 }} onClick={() => { setForumPosts(prev => prev.map(p => p.id === post.id ? { ...p, votes: p.votes - 1 } : p)); apiFetch("/api/muse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "forum", type: "vote", postId: post.id, direction: "down" }) }).catch(() => {}); }}>▼</button>
                  </div>

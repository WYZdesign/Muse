                           ...
                           {(() => {
                              ...
                              const heroPortrait = !!PORTRAIT_IMG[heroSrc];
                               return (
                                ...
                                <div className="card-info-scroll" ref={cardScrollRef} onScroll={(e)=>{if(isTop){const scrollY=(e.target as HTMLElement)?.scrollTop||0;setCardScrolled(scrollY>10);}}}>
                                  ...
                                  {isTop && (
                                    <div className={"match-fab"+(cardScrolled?" hidden":"")}>
                                      <button className={"match-fab-btn"+(showMatchMenu?" open":"")} onClick={()=>setShowMatchMenu(v=>!v)} aria-label="Match actions" style={{fontFamily:showMatchMenu?"inherit":"'Playfair Display',serif",fontStyle:showMatchMenu?"normal":"italic",lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>{showMatchMenu ? <FiCamera size={24} color="#0a0612" /> : "M"}</button>
                                      <div className={"match-radial"+(showMatchMenu?" open":"")}>
                                       <button className="match-radial-btn btn-rewind" style={{left:-33,top:52}} onClick={doRewind} aria-label="Rewind">↺</button>
                                       <button className="match-radial-btn btn-nope" style={{left:-47,top:10}} onClick={()=>doSwipe("left")} aria-label="Pass">✕</button>
                                       <button className="match-radial-btn btn-super" style={{left:-29,top:-29}} onClick={()=>doSwipe("super")} aria-label="Super Like">★</button>
                                       <button className="match-radial-btn btn-like" style={{left:10,top:-47}} onClick={()=>doSwipe("right")} aria-label="Like">♥</button>
                                       <button className="match-radial-btn btn-note" style={{left:52,top:-33}} onClick={doLikeWithNote} aria-label="Like + Note">✎</button>
                                      </div>
                                   </div>
                                  )}
                                 </>
                ...
                <div style={{margin:"0 20px 12px",padding:"12px 0",display:"flex",gap:10,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1,display:"flex",flexDirection:"column",gap:8}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center",width:"100%"}}>
                      ...
                      {feedMedia.slice(0,1).map((url,i)=><div key={i} style={{position:"relative",width:36,height:36,flexShrink:0}}>{url.endsWith(".mp4")||url.includes("video")?<video src={url} style={{width:36,height:36,borderRadius:8,objectFit:"cover"}} />:<img loading="lazy" src={url} alt="" style={{width:36,height:36,borderRadius:8,objectFit:"cover"}} />}<button onClick={()=>setFeedMedia(prev=>prev.filter((_,j)=>j!==i))} style={{position:"absolute",top:-4,right:-4,width:16,height:16,borderRadius:"50%",background:"var(--coral)",border:"none",color:"#fff",fontSize:10,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><FiX size={10} /></button></div>)}
                      <button className="btn btn-gold" style={{flex:1,padding:"10px 0",fontSize:13,fontWeight:700,borderRadius:12,whiteSpace:"nowrap"}} onClick={async()=>{if(feedText.trim()||feedMedia.length){const txt=feedText.trim();const hasVideo=feedMedia.some(u=>u.endsWith(".mp4")||u.includes("video"));const type=feedMedia.length?hasVideo?"video":"photo":"text";setFeedText("");setFeedMedia([]);setFeedPosts(prev=>[{id:uid(),author:currentUser.name,avatar:currentUser.avatar,type,text:txt,likes:0,comments:0,shares:0,time:"Just now",img:feedMedia[0]||undefined,media:feedMedia,liked:false,saved:false,reactions:{}},...prev]);try{await apiFetch("/api/muse",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"feed",text:txt,media:feedMedia,userId:currentUser.id})});showToast("Posted!")}catch{showToast("Failed to post")}}}}>Post</button>
                      <button className="btn btn-outline" style={{flex:1,padding:"10px 0",fontSize:13,fontWeight:600,borderRadius:12,whiteSpace:"nowrap"}} onClick={async()=>{if(feedText.trim()||feedMedia.length){const txt=feedText.trim();setFeedText("");setFeedMedia([]);const moment={id:uid(),author:currentUser.name,avatar:currentUser.avatar,type:feedMedia.length?"photo":"text",text:txt,img:feedMedia[0]||undefined,media:[...feedMedia],time:"Just now"};setStories(prev=>[moment,...prev]);showToast("Moment posted!");}}}>BTS</button>
                    </div>

...
export default function SupportChat({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function send(q?: string) {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "assistant", text: "Something went wrong on my end. Try again in a moment, or email info@wyzdesign.com." }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={PANEL}>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%" }}>
            ...
            {m.sources && m.sources.length > 0 && (
              <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.35)", marginTop: 4, paddingLeft: 4 }}>
        ...
        {loading && (
          <div style={{ alignSelf: "flex-start", color: "rgba(255,255,255,0.5)", fontSize: 13, padding: "8px 14px", background: "rgba(255,255,255,0.06)", borderRadius: "16px 16px 16px 4px" }}>
            Thinking...
          </div>
        )}
        {messages.length <= 1 && !loading && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 4 }}>
            {QUICK_TOPICS.map((t) => (
              <button key={t} onClick={() => send(t)} style={{ padding: "6px 12px", borderRadius: 99, background: "rgba(255,215,0,0.1)", border: "1px solid rgba(255,215,0,0.2)", color: "#FFD700", fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>{t}</button>
            ))}
          </div>
        )}
      </div>

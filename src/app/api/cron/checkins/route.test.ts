...
vi.mock("@/lib/email", () => ({ sendEmail: vi.fn(), notify: vi.fn() }));

import { GET } from "@/app/api/cron/checkins/route";

function req(authorization?: string) {
  return { headers: { get: (name: string) => name === "authorization" ? authorization || null : null } } as any;
}

describe("checkins cron authorization", () => {
  beforeEach(() => {
  ...
  it("does no work in demo mode with valid credentials", async () => {
    ...
    expect(await response.json()).toMatchObject({ success: true, demo: true, checkinsCreated: 0 });
  });
});
